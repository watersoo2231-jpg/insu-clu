  ...
  return (
    <div className="flex-1 flex flex-col px-8 gap-4 justify-center">
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-text-muted text-xs font-medium">
            {installing ? '잠시만 기다려 주세요' : done ? '모든 항목이 준비되었습니다' : failed ? '로그를 확인해 주세요' : '아래 항목을 설치합니다'}
          </p>
        </div>
      </div>
      ...
      <div className="flex gap-3 justify-end mt-1">
        {failed && <Button variant="secondary" size="sm" onClick={runInstall}>다시 시도</Button>}
        {!done && !installing && !failed && (
          <Button variant="primary" size="lg" onClick={runInstall}>설치 시작</Button>
        )}
        {done && <Button variant="primary" size="lg" onClick={onDone}>다음 단계로</Button>}
      </div>
    </div>
  )
}
